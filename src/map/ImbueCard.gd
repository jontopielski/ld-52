...
func _on_Confirm_pressed():
	...
	var upgraded_card = card_to_upgrade.duplicate()
	var positive_effects = get_all_positive_effects()
	randomize()
	positive_effects.shuffle()
	while !positive_effects.empty():
		var next_effect = positive_effects.pop_front()
		if !upgraded_card.effects.has(next_effect):
			upgraded_card.effects.push_back(next_effect)
			break
	Globals.remove_card(card_to_upgrade)
	Globals.deck.push_back(upgraded_card)
	get_tree().call_group("Map", "event_finished")
	$SelectedCard.get_child(0).queue_free()
	queue_free()

func get_all_positive_effects():
	return Globals.get_resources_at_path("res://resources/effects/positive")
